...
def combine_data(data_path):
    splits = {"train": [], "valid": []}
    for folder in ["underwater_dark", "underwater_imagenet", "underwater_scenes"]:
        for data_type in ["trainA"]:
            folder_path = os.path.join(data_path, folder, data_type)
            splits_data = split_data(folder_path, prefix=f"{folder}/trainB/")
            splits["train"].extend(splits_data["train"])
            splits["valid"].extend(splits_data["valid"])
    return splits
...
if __name__ == "__main__":
    ...
    if args.pair:
        ...
        print("Combining all data into a single dataset...")
        splits = combine_data(args.data)
    else:
